...
async def check():
    async with aiohttp.ClientSession() as session:
        for username in usernames:
            async with session.get(f'https://www.tiktok.com/@{username}', headers={'Connection': 'keep-alive', 'User-Agent': 'TikTok 17.4.0 rv:174014 (iPhone; iOS 13.6.1; sv_SE) Cronet'}) as c:
                check = c.status
                if check == 404:
